"""
Top level file. Set up and run the experiments describe in the paper.

We put the functions used for those experiments first. There are some others beneath which were used for preliminary experiments
"""
...
# Top-level function for experiments in paper.
# Run using the best strategy for num_cycles
#
# You can continue the experiment after that using the starts_from_cycle argument.
# However (ugly, sorry), since the name of the experiment directory is f'experiment_async_explicit_with_voting_{num_cycles}',
# you need to change the name of the directory first.
...
# Create the experiment directory and run for num_cycles
...
# Run a single cycle. The CoT few-shot player plays two games against all the other players.
...
# Play a single game and log it in the cycle directory
...
# Create summaries of all the cycles
...
# Extract information from the logs store in an experiment directory.
# Return a tuple ( error_counts, summary_scores, experiment_cost) where
#
# error_counts is a structure with data about per-round correctness of decisions
# summary_scores is a structure with data about per-round scores in the matches
# experiment_cost is the cost of the experiment in dollars
...
# Produce a plot using the error_counts returned by analyze_experiment_results and tracking a smoothed result across rounds
...
# Calculate significance figures using the error_counts returned by analyze_experiment_results,
# contrasting the segment start1 - end1 against start2 - end2
...
# Produce a plot using the summary_scores returned by analyze_experiment_results and tracking a smoothed result across rounds
...
# Calculate significance figures using the summary_scores returned by analyze_experiment_results,
# contrasting the segment start1 - end1 against start2 - end2
...
# More or less obsolete material from preliminary experiments
